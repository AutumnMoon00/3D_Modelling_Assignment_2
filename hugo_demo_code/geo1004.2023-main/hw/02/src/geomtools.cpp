                      ...
                      const std::vector<Point3>& lspts)
{
  ...
  Plane bestfitplane = get_best_fitted_plane(planepts);  // best fitting plane with only outer ring vertices
  ...
  if (pgn.is_simple() == false) {
    return re;  // if polygon is not simple -> empty re is returned
  }
